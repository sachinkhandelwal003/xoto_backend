...
export const uploadDocument = async (req, res) => {
  try {
    const { entityType, entityId, documentType, documentCategory } = req.body;
    const file = req.file;
    if (!file) return res.status(400).json({ success: false, message: "No file uploaded" });
    
    const documentId = `DOC-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`;
    const document = await Document.create({
      documentId, entityType, entityId, documentType, documentCategory,
      fileName: file.originalname, fileSizeMb: file.size / (1024 * 1024), fileUrl: file.path, fileHash: crypto.createHash('md5').update(file.buffer).digest('hex'),
      mimeType: file.mimetype, uploadedBy: { role: req.user?.agentType || 'partner', userId: req.user._id, userName: req.user?.fullName || req.user?.email },
      uploadedFromIp: req.ip, verificationStatus: 'pending', encryption: 'AES-256',
    });
    
    await HistoryService.logDocumentActivity(document, 'DOCUMENT_UPLOADED', await getUserInfo(req), { description: `Document ${document.fileName} uploaded` });
...
export const verifyDocument = async (req, res) => {
  try {
    ...
    await HistoryService.logDocumentActivity(document, 'DOCUMENT_VERIFIED', await getUserInfo(req), { description: `Document ${document.fileName} verified` });
...
export const rejectDocument = async (req, res) => {
  try {
    ...
    await HistoryService.logDocumentActivity(document, 'DOCUMENT_REJECTED', await getUserInfo(req), { description: `Document ${document.fileName} rejected`, notes: reason });
