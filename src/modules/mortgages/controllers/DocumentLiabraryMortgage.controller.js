...
exports.updateDocumentStatus = async (req, res) => {
    try {
        ...
        // If status is Archived, set deleted flag
        if (status === "Archived") {
            document.isDeleted = true;
            document.deletedAt = new Date();
        } else if (status === "Active" && previousStatus === "Archived") {
            // If restoring from Archived, clear deleted flags
        ...
        // Log status change (optional - if you have history service)
        // await HistoryService.logDocumentActivity(document, 'STATUS_CHANGED', await getUserInfo(req), {
...
/**
 * =========================================
 * GET REQUIRED DOCUMENTS (Dynamic)
 * =========================================
 */
    ...
    try {
        ...
        return res.status(200).json({
            ...
            summary: {
                ...
                requiresFrontBack: documents.filter(d => d.requiresFrontBack).length,
                requiresSignature: documents.filter(d => d.requiresSignature).length,
                requiresStamp: documents.filter(d => d.requiresStamp).length,
                directUpload: grouped.direct_upload.length,
                templateDownload: grouped.template_download.length,
                sampleView: grouped.sample_view.length
            }
...
exports.downloadTemplate = async (req, res) => {
    try {
        ...
        await DocumentRequirement.findByIdAndUpdate(id, {
            $inc: { 'template.downloadCount': 1 }
        });
